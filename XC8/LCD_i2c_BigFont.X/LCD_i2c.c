...
void Write_PCF(char dato){
        ...
        //I2C1_Write1ByteRegister( PCF|(PCF_ID),0, (dato | 0b00001000) );
        I2C1_WR(PCF|(PCF_ID),(dato | blacking));
    #else
       i2c_start();
...
void Send_D7_D4(uint8_t address,uint8_t nibble){  
   ...
   //delay_us(60); //original en la lib
   //delay_us(1);//poner por si acaso
...
void lcd_gotoxy(uint8_t x, uint8_t y){
   ...
   //address += x-1; //descomentar para hacer que lcd empiece en 1,1
   address += x;    //descomentar para hacer que lcd empiece en 0,0
   lcd_send_byte(0, 0x80 | address);
   
   //g_LcdX = x - 1;  //descomentar para hacer que lcd empiece en 1,1
   //g_LcdY = y - 1;  //descomentar para hacer que lcd empiece en 1,1
   g_LcdX = x;  //descomentar para hacer que lcd empiece en 0,0
   g_LcdY = y;  //descomentar para hacer que lcd empiece en 0,0
}
